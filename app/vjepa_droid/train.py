...
def main(args, resume_preempt=False):
	...
	frameskip = cfgs_data.get("frameskip", 1)  # Default to 1 for no subsampling
	...
	WANDB_ENABLED = wandb_config.get("enable", False)
	if WANDB_ENABLED:
		wandb.init(
			project=wandb_config.get("project", "vjepa2"),
	...
	print("Finished initializing transforms")
			...
			def log_stats():
				...
				if WANDB_ENABLED:
					wandb.log({
						...
						"wd": _new_wd,
						"lr": _new_lr,
						"gpu_mem": torch.cuda.max_memory_allocated() / 1024.0**2,
						"iter_time_ms": iter_time_meter.avg,
						"gpu_time_ms": gpu_time_meter.avg,
						"data_time_ms": data_elapsed_time_meter.avg,
					})
