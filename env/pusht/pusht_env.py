...
class PushTEnv(gym.Env):
	...
	def reset(self, seed=None, options=None, **kwargs):
		...
		proprio = obs_state[:2]
		if self.with_velocity:
			proprio = np.concatenate((proprio, obs_state[5:]))
		observation = {
			"visual": visual,
			"proprio": proprio
		}
		info = {"state": obs_state}
	...
	def step(self, action):
		...
		truncated = False   # You can add truncation logic if needed
	...
	def _setup(self):
		...
		# self.collision_handeler = self.space.add_collision_handler(0, 0) # NOTE (VG): change made here... how to fix
